...
$enqID = $_GET['enqID'];
$enquiries = mysql_query("SELECT refID FROM `enquiries` WHERE enqID = '$enqID'");
$res = mysql_fetch_array($enquiries);
$refID = $res['refID'];
?>
...
$(document).on('click','#elt1',function() {
	$("#band").show();
});

$(document).on('click','#elt2',function() {
	$("#band").hide();
});
...
function getProcessDetails(id) {	
	$.ajax({
		url: "get_process_details.php?id="+id,
		success: function(data){ $("#processDetails").html(data); }   
	});
}
function disableMe(box) {
	if(box.name == "knowRE" && box.checked) {
		studForm.knowOther.disabled = false;
	} else if(box.name == "knowRE" && box.checked == false) {
		studForm.knowOther.disabled = true;
	}
}
function getCentre(id) { 
	$.ajax({
		url: "get_ieltsCentre.php?id="+id,
		success: function(data){ $("#getCentre").html(data); }   
	});
}
</script>
<?php 
if(!empty($_POST['e-cart-session_data'])){
	$eCartSessionDetails = json_decode($_POST['e-cart-session_data']);
}
?>
<?php
if(isset($_POST['submit'])) {
	...
	if($fname != "firstname") {	
		$students = mysql_query("INSERT INTO `students` (`empID`, `regNo`, `branchID`, `date`, `gender`, `fname`, `lname`, `dob`, `photo`, `married`, `spName`, `spOccupation`, `gdName`, `gdOccupation`, `program`, `subPgm`, `pgmOthers`, `country`, `lastInc`) VALUES ('$empID', '$regNo', '$branchID', '$date', '$gender', '$fname', '$lname', '$dob', '$photo', '$married', '$spName', '$spOccupation', '$gdName', '$gdOccupation', '$program', '$subPgm', '$pgmOthers', '$country', '$counter')");
	...
	if($passNum != '') {
		$passport_details = mysql_query("INSERT INTO `stud_passport` (`studID`, `passNum`, `expiry`, `visaNum`, `fromDate`, `toDate`) VALUES ('$studID', '$passNum', '$expiry', '$visaNum', '$fromDate', '$toDate')");
	}
	
	$knowRE = $_POST['knowRE']; 
	if($knowRE == '3') {
		$iCentre = $_POST['centre']; 
		$referrer = $_POST['referrer']; 
	} else if($knowRE == '6') {
		$knowOthers = $_POST['knowOther']; 
	}
	...
	$write = $_POST['write'];
	$speak = $_POST['speak'];
	if($_POST['iDate'] != '') { $iDate = date("Y-m-d", strtotime($_POST['iDate'])); }
	$proBy = $_POST['proBy'];
	if($proBy == '2') {
	...
	if($knowRE != '') {
		$other_details = mysql_query("INSERT INTO `stud_others` (`studID`, `knowRE`, `knowOthers`, `iCentre`, `referrer`, `engTest`, `iScore`, `listen`, `read`, `write`, `speak`, `iDate`, `proBy`, `assignEmpID`, `suppID`) VALUES ('$studID', '$knowRE', '$knowOthers', '$iCentre', '$referrer', '$engTest', '$iScore', '$listen', '$read', '$write', '$speak', '$iDate', '$proBy', '$assignEmpID', '$suppID')");
	}
	...
	if($students > 0) {
		...
		echo '<META HTTP-EQUIV=Refresh CONTENT="3; URL=student_add.php">';
		?>    
...
<?php if(!empty($eCartSessionDetails)){?>
	<h2>Student Preference</h2>
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	    <td class="red font-12"> Details</td>
	    <td><?php echo $eCartSessionDetails->details;?></td>
	  </tr>
	  <tr>
	    <td class="red font-12"> Program Intake</td>
	    <td><?php echo $eCartSessionDetails->pgmIntake;?></td>
	  </tr>
	  <tr>
	    <td class="red font-12"> Sub Program</td>
	    <td><?php echo $eCartSessionDetails->subPgm;?></td>
	  </tr>
	  <tr>
	    <td class="red font-12"> Program Fees</td>
	    <td><?php echo $eCartSessionDetails->pgmFees;?></td>
	  </tr>
	  <tr>
	    <td class="red font-12"> Institution</td>
	    <td><?php echo $eCartSessionDetails->inst;?></td>
	  </tr>
  </table>
  <br/>
<?php  }?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
  ...
  <?php if($refID != '') { ?>
  <tr bgcolor="#FFFFFF">
    <td height="50" class="red font-14" align="center">Enquiry Id</td>
    <td>:</td>
    <td colspan="4" class="red font-14"><b><?php echo $refID; ?></b></td>
  </tr>
  <?php } ?>
  <tr>
    <td height="30" colspan="6" class="blue font-14">Q. How did you know about Riya Education?</td>
  </tr>
  <tr>
    <td colspan="6">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="pad">
  <tr>
    <td width="25%" height="30"><input type="radio" name="knowRE" id="teleCall" value="1"/> Tele-calling</td>
    <td width="25%"><input type="radio" name="knowRE" id="newspaper" value="2"/> Newspaper</td>
    <td width="15%"><input type="radio" name="knowRE" id="centre" value="3" onclick="getCentre(this.value)"/> IELTS Centre</td>
    <td width="35%"><div id="getCentre"></div></td>
  </tr>
  <tr>
    <td height="30"><input type="radio" name="knowRE" id="frnds" value="4"/> Friends/Family</td>
    <td><input type="radio" name="knowRE" id="riyaEmp" value="5"/> Riya Employee</td>
    <td>
    <input type="radio" name="knowRE" id="others" onclick="disableMe(this)" value="6"/> Others	</td>
    <td><input type="text" name="knowOther" id="knowOther" class="textbox" disabled="disabled"/></td>
  </tr>
</table></td>
  </tr>
  <tr>
    <td colspan="6" height="60"><h6>Basic Info</h6></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>:</td>
    <td>
    <input name="married" id="ms1" type="radio" value="0" checked="checked" /> No
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <input name="married" id="ms2" type="radio" value="1" /> Yes    </td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <?php
	$res1 = mysql_query("SELECT * FROM `programs` WHERE delStatus = '0' ORDER BY program ASC");
  ...
  <tr>
    ...
    <span class="red">Q. Have you ever taken an English language test?</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="radio" name="engTest" id="elt1" value="1" />
      Yes&nbsp;&nbsp;&nbsp;
      <input type="radio" name="engTest" id="elt2" value="2" /> 
      No</td>
  </tr>
  <tr>
    <td colspan="6">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="pad">
        <tr>
          ...
          <td width="2%">:</td>
          <td width="66%">Overall 
            <input type="text" name="iScore" id="iScore" class="textbox width_60 textCenter"/> 
            &nbsp; L &nbsp;
            <input type="text" name="listen" id="listen" class="textbox width_60 textCenter"/>
            &nbsp; R &nbsp;
            <input type="text" name="read" id="read" class="textbox width_60 textCenter"/>
            &nbsp; W &nbsp;
            <input type="text" name="write" id="write" class="textbox width_60 textCenter"/>
            &nbsp; S &nbsp;
            <input type="text" name="speak" id="speak" class="textbox width_60 textCenter"/></td>
        </tr>
        <tr>
          <td>Date of Examination</td>
          <td>:</td>
          <td><input type="text" name="iDate" id="iDate" class="textbox textCenter"/></td>
        </tr>
  ...
  <tr>
    <td colspan="6"><h2>Processed by</h2></td>
  </tr>
  <tr>
    <td colspan="6" bgcolor="#e1e1e1" height="60"><table width="60%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="24%">&nbsp;</td>
        <td width="38%" class="green">
        <input type="radio" name="proBy" id="riya" value="1" onclick="return getProcessDetails(this.value);"/> 
          <strong>Riya Education</strong></td>
        <td width="38%" class="green">
        <input type="radio" name="proBy" id="supplier" value="2" onclick="return getProcessDetails(this.value);"/> 
          <strong>Supplier</strong></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td colspan="6"><div id="processDetails"></div></td>
  </tr>
  <tr>
