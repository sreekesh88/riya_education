...
while($array = mysql_fetch_array($query)) {
	...
	$studID = $array['studID'];
	$pgmID = $array['program'];
	$pgmOthers = $array['pgmOthers'];
	$conID = $array['country'];
	$convertStatus = $array['convertStatus'];	
	
	$activeEmpCode = $array['empCode'];
}
...
function change_status(id) { 
	...
	pop_win = window.open('change_status.php?id='+id, '','left='+left+',top='+top+',toolbar=no,menubar=no,scrollbars=yes,width=400,height=200');
}
</script>
...
if(isset($_POST['submit'])) {
	...
	if($notes != 'Enter your notes here...') {
		$query = mysql_query("INSERT INTO `follow_up` (`empCode`, `studID`, `notes`, `date`, `time`) VALUES ('$empCode', '$studID', '$notes', '$date', '$time')");
	}	
}	
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	while($ary = mysql_fetch_array($result)) {
		...
		$employeeCode = $ary['empCode'];
		
		$res = mysql_query("SELECT fname,lname FROM `employees` WHERE empCode = '$employeeCode'");
