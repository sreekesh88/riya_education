...
if(isset($_POST['submit'])) {
	...
	if($docID != '') {								
		$query = mysql_query("UPDATE `stud_docs` SET `otherDoc`='".$newDoc."',
								`uploads`='".$uploads."',
								`date`='".$date."'
								
								WHERE studID = '$studID' AND docID = '$docID'");
	}
	echo '<META HTTP-EQUIV=Refresh CONTENT="1; URL=stud_documents.php?sid='.$studID.'">';
}
?>
